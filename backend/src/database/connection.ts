...
class Database {
  ...
  async queryWithRetry<T extends QueryResultRow = any>(
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (
          error.code === '23505' || // Violation de contrainte unique
   ...
   */
  private async initializeCustomFunctions(): Promise<void> {
    try {
      ...
      logger.debug('Custom database functions initialized');
    } catch (error) {
      logger.warn('Error initializing custom functions:', error);
    }
  }
...
export const connectDatabase = new Database();

// Helper pour obtenir le pool directement
export const getDb = () => connectDatabase.getPool();
